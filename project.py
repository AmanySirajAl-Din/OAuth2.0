...
def gconnect():
    ...
    # confirm that the token that the client sends to the server
    # matches the token that the server sent to the client
    if request.args.get('state') != login_session['state']:
    # request.args.get => examines the state token passed in
    # and compares it to the state of the login session
    # if not matched =>
        response = make_response(json.dumps('Invalid state parameter.'), 401)
        # return Invalid state parameter message
        response.headers['Content-Type'] = 'application/json'
        # this round trip vrification helps ensure that
        # the user is making the request an not a malicious script
        # (Authentication)
        return response
